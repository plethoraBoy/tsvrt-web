          ...
          <div className="hidden lg:flex lg:gap-x-12">
            {menuItems.map((item, index) => (
              <a
                key={`${item.name}-${index}`}
                href={`#${item.href}`}
                className={`text-sm transition-all duration-300 font-semibold hover:text-yellow-200 hover:font-extrabold   leading-6 ${isNavbarFixed ? "text-white" : "text-white-900"}`}
